...
public class Clan {
    ...
    public static void applyClanTempEffect(Clan clan, int optionId, int param, long durationMillis) {
        long expireTime = System.currentTimeMillis() + durationMillis;
        for (ClanMember member : clan.getMembers()) {
            if (member != null) {               
                PlayerDAO.saveTempEffect(member.id, optionId, param, expireTime);
                Service.gI().sendThongBao((List<Player>) member, "Bạn nhận được buff +" + param + "% chỉ số trong 7 ngày");
            }
        }
    }
...
//
    ...
    public void insert() {
        ...
        try (Connection con = LocalManager.getConnection();) {
            ps = con.prepareStatement("insert into clan (id, name, name_2, slogan, img_id, power_point, max_member, clan_point, level, members, tops, thongTinLeader) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?)");
            ps.setInt(1, this.id);
    ...
    public void update() {
        ...
        try (Connection con = LocalManager.getConnection();) {
            ps = con.prepareStatement("update clan set slogan = ?, img_id = ?, power_point = ?, max_member = ?, clan_point = ?, "
                    + "level = ?, members = ?, name_2 = ?, tops = ?, thongTinLeader = ? where id = ? limit 1");
