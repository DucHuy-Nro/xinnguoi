        ...
        try {
            ...
            if ((cuocBaHatMit > 0 && !playerWin) || (cuocPlayer > 0 && playerWin)) {
                ...
                if (playerWin) {
                    ...
                    tiLe = tongCuoc / cuocPlayer;
                } else {
                    if (cuocBaHatMit == 0) {
                        return;
                    }
                    tiLe = tongCuoc / cuocBaHatMit;
                }

                for (Player pl : binhChon) {
                    try {
                        int cuoc = playerWin ? pl.binhChonPlayer : pl.binhChonHatMit;
                        if (cuoc > 0 && pl.zoneBinhChon.equals(zone)) {
                            long vangNhan = cuoc * tiLe;
                            pl.inventory.gold += vangNhan;
                            pl.binhChonPlayer = 0;
                            pl.binhChonHatMit = 0;
                            Service.gI().sendMoney(pl);
                            Service.gI().sendThongBao(pl,
                                    "Chúc mừng bạn đã thắng " + cuoc + " bình chọn đúng và được thưởng "
                                    + Util.numberToMoney(vangNhan) + " vàng");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
