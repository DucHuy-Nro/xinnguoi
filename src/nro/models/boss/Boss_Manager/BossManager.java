...
import nro.models.boss.Biet_doi_Rocket.Kochiro;
import nro.models.boss.Biet_doi_Rocket.Meowth;
import nro.models.boss.Biet_doi_Rocket.Musashi;
...
public class BossManager implements Runnable {
    ...
    public void loadBoss() throws Exception {
    ...
    public void run() {
        while (ServerManager.isRunning) {
            try {
                ...
                Iterator<Boss> iterator = this.bosses.iterator();
    ...
    public void spawnBietDoiHoaTien() throws Exception {
        int[] mapJoin = BossesData.BIET_DOI_HOA_TIEN[0].getMapJoin();
        int mapId = mapJoin[Util.nextInt(mapJoin.length)];
        Zone zone = MapService.gI().getMapById(mapId).getZone(0);

        int baseX = 500;
        int baseY = 300;

        Boss musashi = new Boss(BossID.MUSASHI, BossesData.BIET_DOI_HOA_TIEN[0]);
        musashi.zone = zone;
        musashi.location.x = baseX;
        musashi.location.y = baseY;
        this.addBoss(musashi);

        Boss kochiro = new Boss(BossID.KOCHIRO, BossesData.BIET_DOI_HOA_TIEN[1]);
        kochiro.zone = zone;
        kochiro.location.x = baseX + 50;
        kochiro.location.y = baseY;
        this.addBoss(kochiro);

        Boss meowth = new Boss(BossID.MEOWTH, BossesData.BIET_DOI_HOA_TIEN[2]);
        meowth.zone = zone;
        meowth.location.x = baseX + 100;
        meowth.location.y = baseY;
        this.addBoss(meowth);
    }
