...
public class Zone {
        ...
        try {
            ...
            for (int i = 0; i < this.items.size(); i++) {
                try {
                    ...
                    Logger.logException(Zone.class, e, "Lỗi xử lý từng item");
                }
            }

            for (int i = removeIndexes.size() - 1; i >= 0; i--) {
                int index = removeIndexes.get(i);
                if (index < items.size()) {
                    items.remove(index);
                }
            }
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            if (plReceive.isPl() && plReceive.getSession() != null && plReceive.getSession().version >= 214) {
                switch (flagbag) {
                    case 83:
                        flagbag = 205;
                        break;
