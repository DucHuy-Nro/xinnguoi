...
public class ConsignShopService {
    ...
    public void upItemToTop(Player pl, int id) {
        ...
        NpcService.gI().createMenuConMeo(pl, ConstNpc.UP_TOP_ITEM, -1, "Bạn có muốn đưa vật phẩm ['" + ItemService.gI().createNewItem(it.itemId).template.name + "'] của bản thân lên trang đầu?\nYêu cầu 50000 vàng.", "Đồng ý", "Từ Chối");
    }

    public void StartupItemToTop(Player pl) {
        final int UP_TOP_FEE = 50000;
        if (pl.inventory.gold < UP_TOP_FEE) {
            Service.gI().sendThongBao(pl, "Bạn cần có ít nhất " + UP_TOP_FEE + " vàng để đưa vật phẩm lên trang đầu");
            return;
        }

        pl.inventory.gold -= UP_TOP_FEE;
        Service.gI().sendMoney(pl);
    ...
    public void claimOrDel(Player pl, byte action, int id) {
        ...
        switch (action) {
            ...
            case 2:
                ...
                if (it.goldSell > 0) {
                    pl.inventory.gold += it.goldSell - it.goldSell * 10 / 100;
                } else if (it.gemSell > 0) {
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            final int CONSIGNMENT_FEE = 1000;

            if (pl.inventory.gold < CONSIGNMENT_FEE) {
                Service.gI().sendThongBao(pl, "Bạn cần có ít nhất " + CONSIGNMENT_FEE + " vàng để làm phí đăng bán");
                return;
            }
            ...
            if (it.template.id == 457) {
                Service.gI().sendThongBao(pl, "Không thể ký gửi Thỏi Vàng.");
                openShopKyGui(pl);
                return;
            }
            ...
            if (money <= 0 || quantity <= 0 || quantity > it.quantity) {
                Service.gI().sendThongBao(pl, "Số lượng hoặc giá tiền không hợp lệ.");
                openShopKyGui(pl);
                return;
            }
            ...
            pl.inventory.gold -= CONSIGNMENT_FEE;
            Service.gI().sendMoney(pl);

            switch (moneyType) {
                case 0:
                    if (money > 200000000) {
                        Service.gI().sendThongBao(pl, "Không thể ký gửi quá 200tr vàng");
                    } else {
