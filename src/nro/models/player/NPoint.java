...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA3) {
            this.player.inventory.itemsBag.stream()
                    .filter(it -> it.isNotNullItem() && it.template.id == 1816)
                    .findFirst()
                    .ifPresent(btc3 -> {
    ...
    private void setHpMax() {
        ...
        //  long hpMax = hpBase + (hpBase * totalPercent / 100L);
        // Xử lý set nappa
        ...
        if (this.player.tempEffects != null) {
            for (TempEffect temp : this.player.tempEffects) {
                if (temp != null && temp.optionId == 77 && temp.isExpired()) {
                    hpMax += (hpMax * temp.param / 100L);
                }
            }
        }
    ...
    private void setMpMax() {
        ...
        if (this.player.tempEffects != null) {
            for (TempEffect temp : this.player.tempEffects) {
                if (temp != null && temp.optionId == 103 && temp.isExpired()) {
                    mpMax += (mpMax * temp.param / 100L);
                }
            }
        }
    ...
    private void setDame() {
        ...
        if (this.player.setClothes.ThanHuyDietChampa == 4) {
            dame += (dame * 20 / 100L);
        }

        if (this.player.setClothes.ThanHuyDietChampa == 5) {
            dame += (dame * 40 / 100L);
        }

        if (this.player.tempEffects != null) {
            for (TempEffect temp : this.player.tempEffects) {
                if (temp != null && temp.optionId == 50 && temp.isExpired()) {
                    dame += (dame * temp.param / 100L);
                }
            }
        }
    ...
    private void setCrit() {
        ...
        if (player.setClothes.ThanHuyDietChampa >= 5) {
            this.crit += 15 / 100;
        }
    }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            if (this.tlTNSM != null) {
                for (Integer tl : new ArrayList<>(this.tlTNSM)) {
                    if (tl != null) {
            ...
            if (this.player.charms.tdDeTu > System.currentTimeMillis()) {
                tiemNang += tn * 2;
            }
            ...
            if (MapService.gI().AllMap(this.player.zone.map.mapId)) {
                //  tiemNang *= 3; //x x3 tiềm năng toàn server
            }
