...
public class UseItem {
    ...
    private static final int ITEM_BAG_TO_SUUTAM = 8;
    private static final int ITEM_SUUTAM_TO_BAG = 9;
        ...
        try {
            ...
            switch (type) {
                ...
                case ITEM_BODY_TO_BOX:
                    ...
                    break;
                case ITEM_BAG_TO_SUUTAM:
                    InventoryService.gI().itemBagToRuongSuuTam(player, 26);
                    break;
                case ITEM_SUUTAM_TO_BAG:
                    InventoryService.gI().itemRuongSuuTamToBag(player, index);
                    break;
                ...
                switch (item.template.type) {
                    ...
                    default:
                        switch (item.template.id) {
                            ...
                            case 1877:
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1782, 1783, 1784);
                                Service.gI().point(pl);
                                break;
                            case 1879:
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1779, 1780, 1781);
                                Service.gI().point(pl);
                                break;
                            case 1226:
                                InventoryService.gI().itemBagToBody(pl, indexBag);
    ...
    private void changePetRamdom(Player pl, Item item) {
        if (!item.hasOption(250, 3000)) {
            Service.gI().sendThongBao(pl, "Cần ít nhất 3000 sức mạnh Kilis để mở!");
            return;
        }

        if (pl.pet == null || pl.pet.typePet != 1 || pl.pet.nPoint.power < 40_000_000_000L) {
            Service.gI().sendThongBao(pl, "Cần có đệ Mabư đạt 40 tỷ sức mạnh để thực hiện!");
            return;
        }

        NpcService.gI().createMenuConMeo(pl, 1200, -1, "Ngươi muốn sở hữu đệ nào?", "Uubb", "Kid Beer", "Kid Jiren", "Từ chối");
    }

    private void changePetRamdomz(Player player, Item item) {
...
//                pl.itemTime.isUseMayDo2 = true;
    ...
    private void useTDLT(Player pl, Item item) {
        if (pl.itemTime.isUseTDLT) {
            ...
            pl.itemTime.timeNotUsingTDLT = System.currentTimeMillis();
        } else {
            ItemTimeService.gI().turnOnTDLT(pl, item);
            pl.itemTime.timeNotUsingTDLT = 0; // reset nếu dùng TDLT
        }
    }
    ...
    public void xoCaxanh(Player pl, Item item) {
        if (InventoryService.gI().getCountEmptyBag(pl) > 0) {
            int[] itemList = {995, 996, 997, 998, 999, 1000, 1001, 1602, 1877, 1879};
            int itemId = itemList[Util.nextInt(0, itemList.length - 1)];

            Item it = ItemService.gI().createNewItem((short) itemId);

            switch (itemId) {
                case 1602:
                    it.itemOptions.add(new ItemOption(50, 21));
                    it.itemOptions.add(new ItemOption(77, 20));
                    it.itemOptions.add(new ItemOption(103, 20));
                    it.itemOptions.add(new ItemOption(8, 2));
                    it.itemOptions.add(new ItemOption(101, 15));
                    it.itemOptions.add(new ItemOption(30, 0));
                    it.itemOptions.add(new ItemOption(231, 0));
                    break;
                case 1877:
                case 1879:
                    it.itemOptions.add(new ItemOption(50, 17));
                ...
                case 996:
                case 997:
                    it.itemOptions.add(new ItemOption(50, Util.nextInt(10, 13)));
                    it.itemOptions.add(new ItemOption(77, Util.nextInt(10, 13)));
                    it.itemOptions.add(new ItemOption(103, Util.nextInt(10, 13)));
                    it.itemOptions.add(new ItemOption(231, 0));
                    break;
                case 998:
                    it.itemOptions.add(new ItemOption(50, Util.nextInt(10, 13)));
                    ...
                    break;
                case 999:
                    it.itemOptions.add(new ItemOption(50, Util.nextInt(11, 14)));
                    it.itemOptions.add(new ItemOption(77, Util.nextInt(9, 13)));
                    it.itemOptions.add(new ItemOption(103, Util.nextInt(9, 13)));
                    it.itemOptions.add(new ItemOption(231, 0));
                    break;
                case 1000:
                    it.itemOptions.add(new ItemOption(50, Util.nextInt(12, 13)));
                    it.itemOptions.add(new ItemOption(77, Util.nextInt(11, 13)));
                    it.itemOptions.add(new ItemOption(103, Util.nextInt(10, 13)));
                    it.itemOptions.add(new ItemOption(231, 0));
                    break;
                case 1001:
                    it.itemOptions.add(new ItemOption(50, Util.nextInt(13, 15)));
                    it.itemOptions.add(new ItemOption(77, Util.nextInt(12, 15)));
                    it.itemOptions.add(new ItemOption(103, Util.nextInt(11, 15)));
                    it.itemOptions.add(new ItemOption(231, 0));
                    break;
            }

            int[] bonusOptions = {14, 94, 97};
            int randomBonusId = bonusOptions[Util.nextInt(0, bonusOptions.length - 1)];
            boolean isDuplicate = it.itemOptions.stream()
                    .anyMatch(opt -> opt.optionTemplate.id == randomBonusId);
            if (!isDuplicate) {
                it.itemOptions.add(new ItemOption(randomBonusId, Util.nextInt(5, 12)));
            }

            ItemOption opt231 = null;
            for (int i = 0; i < it.itemOptions.size(); i++) {
                if (it.itemOptions.get(i).optionTemplate.id == 231) {
                    opt231 = it.itemOptions.remove(i);
                    break;
                }
            }
            if (opt231 != null) {
                it.itemOptions.add(opt231);
            }
    ...
    public void xoCavang(Player pl, Item item) {
        if (InventoryService.gI().getCountEmptyBag(pl) > 0) {
            int[] itemList = {995, 996, 997, 998, 999, 1000, 1001, 1602, 1877, 1879};
            int itemId = itemList[Util.nextInt(0, itemList.length - 1)];
