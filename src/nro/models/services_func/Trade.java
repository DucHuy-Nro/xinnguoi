...
public class Trade {
    ...
    public static final int MAX_GOLD_TRADE_PER_TIME = 10_000;
    public static final long REQUIRED_STRENGTH_FOR_GOLD_TRADE = 40_000_000_000L;
        ...
        if (pl.getSession().actived) {
            if (index == -1) { // Giao dịch vàng
                ...
                if (item.template.id == 457) {
                    if (pl.nPoint.power < REQUIRED_STRENGTH_FOR_GOLD_TRADE) {
                        Service.gI().sendThongBao(pl, "Sức mạnh của bạn phải đạt " + Util.powerFormater(REQUIRED_STRENGTH_FOR_GOLD_TRADE) + " mới có thể giao dịch Thỏi vàng.");
                ...
                if (item.template.id == 1635) {
                    Service.gI().sendThongBao(pl, "Không thể giao dịch");
                    removeItemTrade(pl, index);
                    return;
                }
                ...
                if (isItemCannotTran(item)) {
                    Service.gI().sendThongBao(pl, "Vật phẩm này không thể giao dịch.");
                    removeItemTrade(pl, index);
                } else {
                    if (quantity > 99) {
                        int n = quantity / 99;
