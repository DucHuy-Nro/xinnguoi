...
public class InventoryService {
    ...
    public Item findItemSuuTam(Player player, int tempId) {
        return this.findItem(player.inventory.itemsSuuTam, tempId);
    }
    ...
    public boolean isExistItemSuuTam(Player player, int tempId) {
        return this.isExistItem(player.inventory.itemsSuuTam, tempId);
    }
    ...
    public List<Item> copyItemsSuuTam(Player player) {
        return copyList(player.inventory.itemsSuuTam);
    }
    ...
    public void removeItemSuuTam(Player player, int index) {
        this.removeItem(player.inventory.itemsSuuTam, index);
    }
    ...
    public void subQuantityItemsSuuTam(Player player, Item item, int quantity) {
        subQuantityItem(player.inventory.itemsSuuTam, item, quantity);
    }
    ...
    public static int countItemBag(Player player, int itemTemplateId) {
        int count = 0;
    ...
    public int getCountItemBag(Player player, int itemTemplateId) {
        int count = 0;
        for (Item item : player.inventory.itemsBag) {
            if (item != null && item.template != null && item.template.id == itemTemplateId) {
                count += item.quantity;
            }
        }
        return count;
    ...
    private Item putItemSuuTam(Player player, Item item) {
        for (int i = 0; i < player.inventory.itemsSuuTam.size(); i++) {
            if (!player.inventory.itemsSuuTam.get(i).isNotNullItem()) {
                player.inventory.itemsSuuTam.set(i, item);
        ...
        if (item.getId() == 691 || item.getId() == 692 || item.getId() == 693) {
            ...
            if (itemsBody.get(0).isNotNullItem() || itemsBody.get(5).isNotNullItem()) {
                Service.gI().sendThongBaoOK(player.isPet ? ((Pet) player).master : player,
                        "Vui lòng cởi áo để có thể sử dụng quần đi biển");
                return sItem;
            }
        }
    ...
    public void itemBagToRuongSuuTam(Player player, int index) {
        if (index < 0 || index >= player.inventory.itemsBag.size()) {
            Service.gI().sendThongBao(player, "Không thể thực hiện");
            return;
        }
        ...
        if (item != null && item.isNotNullItem()) {
            if (item.template.id == 457) {
                Service.gI().sendThongBao(player, "Không thể cất vàng vào rương sưu tầm");
                return;
            }
            if (addItemSuuTam(player, item)) {
                if (item.quantity == 0) {
                    Item sItem = ItemService.gI().createItemNull();
                    player.inventory.itemsBag.set(index, sItem);
                }
                sortItems(player.inventory.itemsBag);
                sendItemBags(player);
                sendItemSuuTam(player);
            }
        }
    }

    private boolean isSuuTamItem(Item item) {
        List<Integer> suuTamItemIds = Arrays.asList(5, 11, 23, 24, 25, 27, 32);
        return suuTamItemIds.contains(item.template.id);
    }
    ...
    public void itemRuongSuuTamToBag(Player player, int index) {
        if (index >= 0 && index < player.inventory.itemsBody.size()) {
            Item item = player.inventory.itemsBody.get(index);
            if (item != null && item.isNotNullItem()) {
                player.inventory.itemsBody.set(index, putItemBox(player, item));
            ...
            Service.gI().sendThongBao(player, "Vị trí không hợp lệ!");
        }
    }
    ...
    public void sendItemSuuTam(Player player) {
        Message msg;
        try {
            msg = new Message(103);
            msg.writer().writeByte(0);
            ...
            for (Item it : player.inventory.itemsSuuTam) {
                if (it != null && it.template != null) {
                    filteredItems.add(it);
                }
            }

            msg.writer().writeByte(filteredItems.size());
    ...
    public void openBoxSuuTam(Player player) {
        Message msg;
        try {
            msg = new Message(103);
            msg.writer().writeByte(1);
    ...
    public boolean addItemSuuTam(Player player, Item item) {
        return addItemList(player.inventory.itemsSuuTam, item);
    }
    ...
    public int getIndexItemSuuTam(Player player) {
