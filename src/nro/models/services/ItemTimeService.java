...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseCoBonLa) {
            ...
            long duration = TIME_CO_BON_LA;

            if (player.itemTime.coBonLaExpireTime > currentTime) {
                player.itemTime.coBonLaExpireTime += duration;
            } else {
                player.itemTime.coBonLaExpireTime = currentTime + duration;
            }

            player.itemTime.isUseCoBonLa = true;

            long secondsLeft = (player.itemTime.coBonLaExpireTime - currentTime) / 1000;

            int secondsToSend = (int) Math.min(secondsLeft, 86400);

            sendItemTime(player, 13618, secondsToSend);
        }
    ...
    public void useCoBonLa(Player player) {
        long currentTime = System.currentTimeMillis();
        long extraTime = 30 * 60 * 1000L; // 30 phút

        if (player.itemTime.timeUseCoBonLa > currentTime) {
            player.itemTime.timeUseCoBonLa += extraTime;
        } else {
            player.itemTime.timeUseCoBonLa = currentTime + extraTime;
        }

        player.itemTime.isUseCoBonLa = true;

        int secondsLeft = (int) ((player.itemTime.timeUseCoBonLa - currentTime) / 1000);
        sendItemTime(player, 13618, secondsLeft);
    }
