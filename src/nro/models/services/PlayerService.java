...
public class PlayerService {
    ...
    public void sendWearingBackItem(Player player) {
        try {
            Message msg = new Message(-64);
            msg.writer().writeInt((int) player.id);
            msg.writer().writeShort(player.wearingBackItemId);
            msg.writer().flush();
    ...
    public void playerMove(Player player, int x, int y) {
        ...
        x = Math.max(0, Math.min(x, player.zone.map.mapWidth - 1));
        y = Math.max(0, Math.min(y, player.zone.map.mapHeight - 1));
        player.location.x = x;
        player.location.y = y;
        ...
        if (player.isPl()) {
            try {
                int tileX = player.location.x / 24;
                int tileY = player.location.y / 24;
                int mapTileWidth = player.zone.map.tileMap[0].length;
                int mapTileHeight = player.zone.map.tileMap.length;

                if (tileX >= 0 && tileX < mapTileWidth && tileY >= 0 && tileY < mapTileHeight) {
                    int type = player.zone.map.tileMap[tileY][tileX];
                    player.isFly = type == 0;
                } else {
                    System.err.println("Lỗi tọa độ ngoài giới hạn tileMap: x=" + tileX + ", y=" + tileY + ", mapId=" + player.zone.map.mapId);
                    player.isFly = false;
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void savePlayer(Player player) {
        if (player != null) {
            try {
                ...
                Logger.logException(PlayerService.class, e, "Lỗi khi lưu dữ liệu người chơi " + player.name);
                Service.gI().sendThongBao(player, "Lỗi khi lưu dữ liệu, vui lòng liên hệ bố mr blue");
            }
        }
    }
    ...
    public void dailyLogin(Player player) {
        if (Util.compareDay(Date.from(Instant.now()), player.firstTimeLogin)) {
            ...
            player.lastCheck = null;
            LocalDate VIP_SEASON_END_DATE_FOR_CURRENT_YEAR = LocalDate.of(LocalDate.now().getYear(), Month.AUGUST, 5);
