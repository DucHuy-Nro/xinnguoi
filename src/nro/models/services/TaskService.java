    ...
    public boolean checkDoneTaskTalkNpc(Player player, Npc npc) {
        switch (npc.tempId) {
            ...
            case ConstNpc.BARDOCK -> {
                return doneTask(player, ConstTask.TASK_29_2) || doneTask(player, ConstTask.TASK_29_4) || doneTask(player, ConstTask.TASK_29_6) || doneTask(player, ConstTask.TASK_29_7) || doneTask(player, ConstTask.TASK_29_8);
            }
        }
        return false;
    }
        ...
        if (!player.isBoss && !player.isBot && !player.isPet) {
            switch (mob.tempId) {
                ...
                case ConstMob.TOBI, ConstMob.CABIRA -> {
                    TaskService.gI().doneTask(player, ConstTask.TASK_29_3);
                    TaskService.gI().doneTask(player, ConstTask.TASK_29_8);
                }
            }
        }
    }
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_29_6:
                    break;
                case ConstTask.TASK_29_7:
                    break;
                case ConstTask.TASK_29_8:
                    break;
                case ConstTask.TASK_29_9:
                    break;
            }
            InventoryService.gI().sendItemBags(player);
            return true;
