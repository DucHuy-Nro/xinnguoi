...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            String itemsSuuTam = dataArray.toJSONString();
            dataArray.clear();
            ...
            LocalManager.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, BoughtSkill, dailyGift, masterDoesNotAttack, data_luyentap, data_achievement, giftcode, total_damage_maydam, data_duahau_egg, nhiem_vu_kol, items_suutam) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, dataBoughtSkill, dailyGift, 0, luyenTapData, achievementData, giftCode, 0, DuaHauEgg, dataKol, itemsSuuTam);
    ...
    public static void insert(Account acc) throws SQLException {
        Connection con = LocalManager.gI().getConnection();
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.idMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                //data suu tam
                for (Item item : player.inventory.itemsSuuTam) {
                    JSONArray opt = new JSONArray();
                    if (item.isNotNullItem()) {
                        dataItem.add(item.template.id);
                        dataItem.add(item.quantity);
                ...
                dataArray.add(player.lastCheck != null ? player.lastCheck.toString() : null);
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "firstTimeLogin = ?,  dailyGift = ?, point_sukien = ?, thachdauwhis = ?, point_sukien1 = ?, point_maydam = ?, total_damage_maydam = ?, data_duahau_egg = ?, checkNhanQua = ?, nhiem_vu_kol = ?, point_sukien2 = ?, items_suutam = ? where id = ?";
    ...
    public static void saveTempEffect(long playerId, int optionId, int param, long expireTime) {
        try (Connection con = LocalManager.gI().getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO temp_effects(player_id, option_id, param, expire_time) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE option_id=?, param=?, expire_time=?")) {
    ...
    public static boolean isExist(String username) {
        ...
        ResultSet rs = null;
        try {
            conn = LocalManager.gI().getConnection();
            ps = conn.prepareStatement("SELECT 1 FROM account WHERE username = ? LIMIT 1");
