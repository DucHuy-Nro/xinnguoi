...
public class Osin extends Npc {
            ...
            switch (this.mapId) {
                ...
                case 52 -> {
                    ...
                    if (TimeUtil.isMabu14HOpen()) {
                        if (hasEnergyJar) {
                            this.createOtherMenu(player, ConstNpc.MABU_2H_OPEN,
                            ...
                            this.createOtherMenu(player, ConstNpc.MABU_2H_NOT_OPEN,
                                    "Mabư đã thoát khỏi vỏ bọc\nmau đi cùng ta ngăn chặn hắn lại\ntrước khi hắn tàn phá trái đất này",
                                    "OK", "Từ chối");
                        }
                    } else if (TimeUtil.isMabuOpen()) {
                        if (hasEnergyJar) {
                            this.createOtherMenu(player, ConstNpc.MABU_12H_OPEN,
                                    "Bây giờ tôi sẽ bí mật...\nđuổi theo 2 tên đồ tể...\nQuý vị nào muốn đi theo thì xin mời !",
                                    "OK", "Bình hút năng lượng", "Từ chối");
                        } else {
                            this.createOtherMenu(player, ConstNpc.MABU_12H_NOT_OPEN,
                                    "Bây giờ tôi sẽ bí mật...\nđuổi theo 2 tên đồ tể...\nQuý vị nào muốn đi theo thì xin mời !",
                                    "OK", "Từ chối");
                        }
                    } else {
    ...
    public void confirmMenu(Player player, int select) {
        ...
        switch (mapId) {
            ...
            case 52 -> {
                switch (indexMenu) {
                    case ConstNpc.MABU_2H_OPEN, ConstNpc.MABU_2H_NOT_OPEN -> {
                        if (select == 0) {
                            if (TimeUtil.isMabu14HOpen()) {
                                MajinBuu14HService.gI().joinMaBu2H(player);
                            } else {
                                Service.gI().sendThongBao(player, "Chưa đến giờ mở map 14h!");
                            }
                        } else if (select == 1) {
                    ...
                    case ConstNpc.MABU_12H_OPEN, ConstNpc.MABU_12H_NOT_OPEN -> {
                        if (select == 0) {
                            if (TimeUtil.isMabuOpen()) {
                                ChangeMapService.gI().changeMap(player, 114, -1, Util.nextInt(100, 500), 336);
                            } else {
                                Service.gI().sendThongBao(player, "vui lòng đợi 12h!");
                            }
                        } else if (select == 1) {
                            createOtherMenu(player, ConstNpc.BINH_HUT_NANG_LUONG,
                                    "Cadic đã bị phù thủy Babidi thôi miên\nhãy mang bình hút năng lượng đến đây\nhút cạn năng lượng tà ác trong cậu ấy",
                            ...
                            switch (selectIndex) {
                                case 0 -> { // Phù hộ 10 ngọc
                                    if (player.inventory.getGem() < 10) {
                                        ...
                                        Service.gI().sendThongBao(player, "Thần Kaio đã phù hộ bạn!");
                                    }
                                }

                                case 1 -> { // Từ chối
                                    Service.gI().sendThongBao(player, "Hẹn gặp lại!");
                                }
                                ...
                                case 2 -> {
                                    if (!player.isPhuHoMapMabu) {
                                        ...
                                        Service.gI().sendThongBao(player, "Không thể quay về trong trạng thái được phù hộ!");
                                    }
                                }
                            }
                        }
